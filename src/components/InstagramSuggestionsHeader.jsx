...
const InstagramSuggestionsHeader = () => {
    ...
    {
      username: "gawiel",
      followedBy: ["Oempress", "0empress_b"],
      isVerified: false,
      avatar: "https://randomuser.me/api/portraits/women/1.jpg"
    },
    // Add 7 more suggestions
    ...Array(7).fill(0).map((_, i) => ({
      username: `user_${i+4}`,
      followedBy: [`follower_${i+1}`, `follower_${i+2}`],
      isVerified: Math.random() > 0.7,
      avatar: `https://randomuser.me/api/portraits/${Math.random() > 0.5 ? 'men' : 'women'}/${i+3}.jpg`
    }))
  ];
    return (
            ...
            <CarouselItem key={index} className="pl-1 basis-2/5">
              <Card className="border-0 rounded shadow-none p-0">
                <CardContent className="flex flex-col items-center p-4">
                  ...
                  <Button variant="outline" size="sm" className="w-full text-xs text-blue-500 hover:text-blue-600 hover:bg-blue-400/5">
                    Follow
                  </Button>
                </CardContent>
              </Card>
